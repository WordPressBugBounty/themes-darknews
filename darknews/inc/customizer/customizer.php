<?php
/**
 * DarkNews Theme Customizer
 *
 * @package DarkNews
 */

if (!function_exists('darknews_get_option')):
/**
 * Get theme option.
 *
 * @since 1.0.0
 ...
 */
function darknews_get_option($key) {
	...
	$default       = darknews_get_default_theme_options();
	$default_value = null;
 ...
 */
function darknews_customize_register($wp_customize) {
    ...
    $wp_customize->get_control('background_color')->label = __( 'Body Background Color (Dark Mode)', 'darknews' );

	if (isset($wp_customize->selective_refresh)) {
		$wp_customize->selective_refresh->add_partial('blogname', array(
				'selector'        => '.site-title a',
				'render_callback' => 'darknews_customize_partial_blogname',
			));
		$wp_customize->selective_refresh->add_partial('blogdescription', array(
				'selector'        => '.site-description',
				'render_callback' => 'darknews_customize_partial_blogdescription',
			));
	}
    ...
    // use get control
    $wp_customize->get_control( 'header_textcolor')->label = __( 'Site Title/Tagline Color', 'darknews' );
    ...
    $wp_customize->register_section_type( 'DarkNews_Customize_Section_Upsell' );

    // Register sections.
    $wp_customize->add_section(
        new DarkNews_Customize_Section_Upsell(
            $wp_customize,
            'theme_upsell',
            array(
...
add_action('customize_register', 'darknews_customize_register');
 ...
 * @return void
 */
function darknews_customize_partial_blogname() {
	bloginfo('name');
}
 ...
 * @return void
 */
function darknews_customize_partial_blogdescription() {
 ...
 */
function darknews_customize_preview_js() {
	wp_enqueue_script('darknews-customizer', get_template_directory_uri().'/js/customizer.js', array('customize-preview'), '20151215', true);

}

add_action('customize_preview_init', 'darknews_customize_preview_js');


function darknews_customizer_css() {
    wp_enqueue_script( 'darknews-customize-controls', get_template_directory_uri() . '/assets/customizer-admin.js', array( 'customize-controls' ) );

    wp_enqueue_style( 'darknews-customize-controls-style', get_template_directory_uri() . '/assets/customizer-admin.css' );
}
add_action( 'customize_controls_enqueue_scripts', 'darknews_customizer_css',0 );
